...
function privatize_histogram(model) {
    ...
    // Apply invariants. Find the lowest number.
    ...
    var lowest_count = model.noisy_counts.reduce(function(a, b) {
        return Math.min(a, b);
    });

    if (lowest_count < 0) {
        model.noisy_counts = model.noisy_counts.map(function(a) {
            return a - lowest_count;
        });
    }


    // - Compute the difference between the total counts and the real total counts
    totalNoisyCounts = model.noisy_counts.reduce(function(a, b) {
        return a + b;
    });
    delta = sgn(totalCounts - totalNoisyCounts);

    console.log("raised floor: ",model.noisy_counts, "totalNoisyCounts:", totalNoisyCounts, "delta:",delta);


    // - Randomly add that to all values until the error is distributed. Don't make any values zero.
    var loop=1000;
    while (totalCounts != totalNoisyCounts && loop>0) {
        console.log("totalCounts="+totalCounts+"  totalNoisyCounts="+totalNoisyCounts+"  delta="+delta);
